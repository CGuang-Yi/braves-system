...
function submitMedical() {
  ...
  // MR (medical review) is a same-day in-camp visit with no duration window, so
  // it doesn't require an end date even though it carries a status enum value.
  const needsDuration = !noDurationStatuses.includes(status) && type !== "MR";
  if (needsDuration && !endIso) { alert("End date is required for all statuses except Pending and NIL."); return; }
  ...
  // dropdown is left on "Auto", derive from the reason; otherwise honour the
  // commander's explicit choice.
  ...
  const entry = {
    ...
    type,
    urtiType,
    mrTiming: type === "MR" ? gv("f-mr-timing").trim() : "",
    // Preserve an existing visit grouping on edit; single-status rows stay blank
    // (the multi-status sibling UI that sets visitId lands in Step 3).
