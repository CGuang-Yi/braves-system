...
// ─── DUTY MESSAGES (MD_Docs/DUTY_LIST_SPEC.md §10) ──────
//
// Both replace a message the duty spreadsheet generated by formula, and both
// follow the same editable-textarea + Copy pattern as everything above.

// The latest date carrying ANY assignment — how far planning has actually
// reached.
//
// The source sheet computed this as TODAY(), which reports today's date whether
// planning ran a month ahead or stopped last week. That is worse than no
// number: it reads as a claim about coverage that nothing checks. Empty means
// nothing is planned at all, which the message says outright rather than
// dressing up as a date.
function dutyPlannedTill() {
  let max = "";
  (STATE.duty || []).forEach(r => {
    if (r && r.d4 && r.date && r.date > max) max = r.date;
  });
  return max;
}

function generateDutyReminder() {
  const cfg = dutyConfig();
  const till = dutyPlannedTill();
  const head = till
    ? `Hello everyone! I've planned your duties till ${toDDMMYY(till)}`
    : `Hello everyone! No duties have been planned yet.`;
  return `${head}\n\n${cfg.dutyReminderBody || ""}`.trim();
}

// One line per slot for a chosen date. PDS lines are generated per LIVE platoon
// rather than hardcoded to four — the number and numbering of platoons can
// change, and a hardcoded list silently drops a fifth platoon's duty.
//
// Unfilled slots are printed as "—" rather than omitted. A missing line reads
// as "no such duty today"; an empty one reads as "nobody is on it yet", which
// is the thing worth noticing the night before.
function generateDutyBoard(dateIso) {
  const cfg = dutyConfig();
  const cols = dutyGridColumns(cfg);
  const rows = (STATE.duty || []).filter(r => r && r.date === dateIso);
  const lines = cols.map(c => {
    const hit = rows.find(r => r.dutyType === c.dutyType && (r.platoon || "") === c.platoon);
    return `${c.label}: ${hit && hit.d4 ? mrRankName(hit.d4) : "—"}`;
  });
  const hol = (STATE.holidays || []).find(h => h && h.date === dateIso);
  const holLine = hol ? `\n(${hol.name}${hol.tentative ? " — tentative" : ""})` : "";
  return `*DUTY ${toDDMMYY(dateIso)}*${holLine}\n\n${lines.join("\n")}`;
}
...
function openReportModal(type) {
  ...
  const titleLabel = type === "FP" ? "First Parade State"
    ...
    : type === "DUTYBOARD" ? "Duty Board (one day)"
    : type === "DUTYREMIND" ? "Duty Planning Reminder"
    : "Medical Status List";
  ...
  const isMR = type === "MR";
  const isDutyBoard = type === "DUTYBOARD";
  // The reminder has no date input at all: "planned till" is DERIVED from the
      ...
      <div style="display:flex;flex-direction:column;gap:10px">
        <div style="font-size:11px;color:var(--muted);background:var(--surface2);border:1px solid var(--border);border-radius:6px;padding:6px 10px">
          ${isConduct
            ...
            : isDutyRemind
            ? `"Planned till" is read from the furthest date carrying an assignment — it is not today's date, so it only moves when planning actually moves. The textarea is editable; <strong>Open in WhatsApp</strong> sends what is in it.`
        ...
        ${isDutyRemind ? "" : `<div class="form-row">
          ${formField("rep-date", "Date", "date", "", dateExtra)}
          ${isDutyBoard ? "" : formField("rep-time", "Time (HHMM)", "text", "0700", timeExtra)}
        </div>`}
        ...
        <!-- white-space:pre everywhere else, because those messages are
             column-aligned and wrapping would break the format. The duty
             reminder is prose, so it wraps. -->
        <textarea id="rep-text" rows="${isDutyRemind ? 8 : 20}" spellcheck="false" style="width:100%;padding:10px;border-radius:6px;border:1px solid var(--border);background:var(--bg);color:var(--text);font-family:'JetBrains Mono',monospace;font-size:11px;line-height:1.45;resize:vertical;white-space:${isDutyRemind ? "pre-wrap" : "pre"}"></textarea>
        <button type="button" id="rep-copy-btn" class="btn btn-success" onclick="copyReportToClipboard()">📋 Copy to Clipboard</button>
        ${isDutyRemind ? `<button type="button" class="btn" onclick="openDutyReminderInWhatsApp()">💬 Open in WhatsApp</button>` : ""}
      </div>
    </form>
  `);
...
// The source sheet's A17 hyperlink, kept. Reads the TEXTAREA rather than
// regenerating, so any last-minute edit the planner made is what gets sent —
// the whole point of the message being editable.
function openDutyReminderInWhatsApp() {
